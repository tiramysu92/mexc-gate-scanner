...
def main():
    ...
    try:
        for line in (root/'SHA256SUMS_V2412.txt').read_text().splitlines():
            expected,name=line.split('  ',1)
            if Path(name).name!=name or hashlib.sha256((root/name).read_bytes()).hexdigest()!=expected:
                raise ValueError(name)
    except (OSError,ValueError) as exc:
        print('Paquet V2.4.12 incomplet ou modifié :',str(exc),file=sys.stderr);return 1
    with tempfile.TemporaryDirectory(prefix='mexc_v2412_tests_') as test_root:
        for suite in SUITES:
            result=subprocess.run([sys.executable,str(root/suite)],cwd=test_root)
            if result.returncode:return result.returncode
    print('V2.4.12 : toutes les suites réussies. Aucun ordre réel envoyé ; aucun réarmement par les tests.')
    return 0
