...
spec=importlib.util.spec_from_file_location('restart',Path(__file__).resolve().parent/'relancer_mexc_v2411.py')
...
def data():
 ...
 if VERSION.startswith('2.4.11') and os.environ.get('FAKE_DIFFERENT')=='1':lv['cap_usd']=999
...
if __name__=='__main__':
 if VERSION.startswith('2.4.11') and os.environ.get('FAKE_EXIT')=='1':raise SystemExit(3)
...
class RestartTests(unittest.TestCase):
 ...
 def start(self,**flags):
  ...
  (self.root/'app.py').write_text(FAKE.replace('__VERSION__','2.4.10-fair-public-dispatch'))
 ...
 def run_restart(self,regularise_cto=False):
  ...
  with contextlib.redirect_stdout(self.out),contextlib.redirect_stderr(self.out), \
   ...
   return m.restart(self.old.pid,self.root,sys.executable,drain_timeout=1.2,stop_timeout=2,startup_timeout=2,regularise_cto=regularise_cto)
 def tearDown(self):
  for child in self.children:
   if child.poll() is None:child.terminate()
   child.wait(timeout=3)
  os.chdir(self.cwd);self.tmp.cleanup()
 def test_restart_preserves_env_and_leaves_stop_and_arm_unchanged(self):
  self.start();mtime=self.arm.stat().st_mtime_ns;d=self.run_restart()
  self.assertEqual(d['version'],m.VERSION);self.assertFalse(d['live']['arm']['effective'])
  self.assertTrue(self.stop.exists());self.assertIsNotNone(self.old.poll());self.assertEqual(self.arm.stat().st_mtime_ns,mtime)
  newpid=int((self.root/'scanner_v2411_live.pid').read_text())
 ...
 def test_busy_trade_prevents_termination(self):
  ...
  self.assertFalse((self.root/'scanner_v2411_live.pid').exists())
 ...
 def seed_cto_journal(self):
  ...
  import collecte_incident_cto as collector
  parameters=patch.multiple(accounting,**SYNTHETIC_PARAMETERS)
  parameters.start();self.addCleanup(parameters.stop)
  selected_attempt=patch.object(collector,'ATTEMPT',SYNTHETIC_PARAMETERS['ATTEMPT'])
  selected_attempt.start();self.addCleanup(selected_attempt.stop)
 ...
 def test_cto_reconciliation_is_applied_between_processes_with_stop_retained(self):
  import sqlite3
  self.seed_cto_journal();self.start(FAKE_CIRCUIT='1');d=self.run_restart(regularise_cto=True)
  with sqlite3.connect(self.root/'local.db') as con:
   status,pnl=con.execute('SELECT status,realized_pnl FROM live_attempts_v240').fetchone()
   self.assertEqual(status,'manual_closed');self.assertAlmostEqual(pnl,-12.004)
  self.assertTrue(self.stop.exists());self.assertTrue(d['live']['circuit_open'])
  self.assertEqual(len(list(self.root.glob('incident_cto_complet_*.json.gz'))),1)
 def test_incorrect_cto_journal_prevents_process_termination(self):
  import sqlite3
  self.seed_cto_journal()
  with sqlite3.connect(self.root/'local.db') as con:con.execute('UPDATE live_open_exposures_v246 SET units=0')
  self.start(FAKE_CIRCUIT='1')
  with self.assertRaises(RuntimeError):self.run_restart(regularise_cto=True)
  self.assertIsNone(self.old.poll());self.assertFalse(self.stop.exists())
