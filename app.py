...
# ============================================================
# MEXC <-> Gate spot arbitrage scanner V3.1
# Public market data only. No API keys and no order execution.
#
# Architecture:
# - Gate BBO: WebSocket, update speed offered by Gate: 10 ms
# - MEXC BBO: one all-symbol REST request, default interval 250 ms
#   (avoids 1 sequential request per token and the V2 ~40 s cycle)
...
def mexc_bbo_loop():
    # One request returns all MEXC symbols, eliminating V2's sequential per-pair scan.
    # V3.1: enforce a real cadence and back off automatically on HTTP 403/429.
    backoff = MEXC_BACKOFF_INITIAL
    last_success_ms = None
    while True:
        loop_started = time.perf_counter()
        try:
            req_started = time.perf_counter()
            arr=fetch_json(MEXC+"/api/v3/ticker/bookTicker", timeout=2.5)
            latency_ms=round((time.perf_counter()-req_started)*1000,1)
            ts=now_ms()
            actual_cycle_ms = (ts-last_success_ms) if last_success_ms is not None else None
            with state_lock:
                wanted=set(state["pairs"])
                for x in arr:
                    sym=x.get("symbol")
                    if sym in wanted:
                        try:
                            state["mexc"][sym]={
                                "bid":float(x["bidPrice"]), "bidq":float(x["bidQty"]),
                                "ask":float(x["askPrice"]), "askq":float(x["askQty"]), "ts":ts
                            }
                        except: pass
                state["last_mexc_ms"]=ts
                state["mexc_cycle_ms"]=actual_cycle_ms
                state["mexc_latency_ms"]=latency_ms
            last_success_ms=ts
            backoff=MEXC_BACKOFF_INITIAL
            scan_candidates(ts)
        except requests.HTTPError as e:
            status = e.response.status_code if e.response is not None else None
            err(f"MEXC BBO HTTP {status}: {e}")
            if status in (403, 429):
                err(f"MEXC backoff {backoff:.1f}s")
                time.sleep(backoff)
                backoff=min(backoff*2, MEXC_BACKOFF_MAX)
            else:
                time.sleep(1.0)
        except Exception as e:
            err(f"MEXC BBO: {e}")
        ...
        # Always enforce the minimum interval between loop starts.
        elapsed=time.perf_counter()-loop_started
        if elapsed<MEXC_BBO_INTERVAL:
            time.sleep(MEXC_BBO_INTERVAL-elapsed)
...
<div class="top">
 ...
 <div class="card kpi"><b>{{cycle}} ms</b><div class="muted">cycle MEXC réel</div></div>
 <div class="card kpi"><b>{{latency}} ms</b><div class="muted">latence HTTP MEXC</div></div>
...
def home():
    ...
    with state_lock:
        ...
        cycle=state["mexc_cycle_ms"]; latency=state["mexc_latency_ms"]; gc=state["gate_connected"]
    ...
    return render_template_string(HTML,pairs=pairs,mexc_age=(ts-lm if lm else "-"),
        gate_age=(ts-lg if lg else "-"),cycle=(cycle if cycle is not None else "-"),latency=(latency if latency is not None else "-"),opps=sum(x["opportunities"] for x in stats),
...
def api_state():
    ...
    with state_lock:
        return jsonify({
            "version":"3.1","pairs":state["pairs"],"pair_count":len(state["pairs"]),
            ...
            "mexc_cycle_ms":state["mexc_cycle_ms"],"mexc_latency_ms":state["mexc_latency_ms"],"gate_connected":state["gate_connected"],
...
def main():
    ...
    # Retry universe discovery before launching feeds.
    while True:
        try:
            discover_universe(); break
        except Exception as e:
            err(f"Universe: {e}"); print("Universe error:",e); time.sleep(3)
    threading.Thread(target=gate_ws_loop,daemon=True).start()
    threading.Thread(target=mexc_bbo_loop,daemon=True).start()
    print(f"[V3] Dashboard http://0.0.0.0:{APP_PORT}")
