...
MEXC_BBO_INTERVAL = float(os.getenv("MEXC_BBO_INTERVAL", "0.10"))       # target 100 ms
    ...
    while True:
        ...
        try:
            ...
            with state_lock:
                ...
                state["mexc_cycle_ms"]=round((time.perf_counter()-started)*1000,1)
            scan_candidates(ts)
        except Exception as e:
            err(f"MEXC BBO: {e}")
            time.sleep(.5)
...
<div class="top">
 ...
 <div class="card kpi"><b>{{cycle}} ms</b><div class="muted">cycle MEXC</div></div>
...
def home():
    ...
    with state_lock:
        ...
        cycle=state["mexc_cycle_ms"]; gc=state["gate_connected"]
    ...
    return render_template_string(HTML,pairs=pairs,mexc_age=(ts-lm if lm else "-"),
        gate_age=(ts-lg if lg else "-"),cycle=cycle,opps=sum(x["opportunities"] for x in stats),
...
def api_state():
    ...
    with state_lock:
        return jsonify({
            "version":3,"pairs":state["pairs"],"pair_count":len(state["pairs"]),
            ...
            "mexc_cycle_ms":state["mexc_cycle_ms"],"gate_connected":state["gate_connected"],
