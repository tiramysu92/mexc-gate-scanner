...
# Primary paper portfolio: first-level BBO only, sequential execution; leg1 at half latency, leg2 at total latency.
...
def reserve_paper_trade(route,event_x,event_start_ts,decision_id,decision_ts):
    ...
    with paper_lock:
        ...
        return {"decision_id":decision_id,"route":route,"event_start_ts":event_start_ts,"t0":decision_ts,
                ...
                "reserved_usd":input_usd,"leg1_due":decision_ts+max(1,PRIMARY_EXEC_LATENCY_MS//2),
                "leg2_due":decision_ts+PRIMARY_EXEC_LATENCY_MS,"stage":1}


def topbook_fill(symbol, frm, to, input_units, books, meta):
    """Execute only against the first visible BBO level. No REST snapshot and no deeper-book assumption."""
    m=meta.get(symbol); b=books.get(symbol)
    if not m or not b or input_units<=0: return None
    age=now_ms()-b["ts"]
    if age>EXEC_OBSERVE_MAX_AGE_MS: return None
    if frm==m["base"] and to==m["quote"]:
        used=min(float(input_units),float(b["bidq"]))
        if used<=0: return None
        out=used*float(b["bid"])*(1.0-FEE)
    elif frm==m["quote"] and to==m["base"]:
        cap=float(b["ask"])*float(b["askq"]); used=min(float(input_units),cap)
        if used<=0: return None
        out=(used/float(b["ask"]))*(1.0-FEE)
    else: return None
    return {"input_used":used,"output":out,"fill_ratio":used/float(input_units),"book":dict(b)}


def paper_leg1(q,ts,books,meta):
    r=q["route"]; sym=r["symbols"][0]; frm,to=r["path"][0],r["path"][1]
    fill=topbook_fill(sym,frm,to,q["reserved_units"],books,meta)
...
def settle_paper_trade(q,exec_ts,books,meta):
    """Sequential first-level-BBO execution. Leg 1 executes first; leg 2 uses the exact intermediate amount obtained."""
    with paper_lock:
        ensure_paper_day(); s,e=q["start_asset"],q["end_asset"]; reserved=q["reserved_units"]
        r=q["route"]; sym2=r["symbols"][1]; mid=r["path"][1]
        fill2=topbook_fill(sym2,mid,e,q["leg1"]["output"],books,meta)
        paper_reserved[s]=max(0.0,paper_reserved.get(s,0.0)-reserved)
        if not fill2:
            paper["skipped_flash"]+=1; persist_paper(); return False
        # With only level 1 available, never invent depth. If leg 2 cannot absorb the whole leg-1 output,
        # classify it as an unresolved/risk execution rather than booking an optimistic PnL.
        if fill2["fill_ratio"]<0.999999:
            paper["skipped_flash"]+=1; persist_paper(); return False
        input_units=min(q["leg1"]["input_used"],paper["balances"].get(s,0.0))
        ...
        net_pct=(output_usd/input_usd-1.0)*100.0 if input_usd else 0.0
        put_db(("paper_trade",(paper["day"],exec_ts,r["id"],s,e,input_units,input_usd,output_units,output_usd,
                                net_pct,profit,q["event_start_ts"])))
...
def schedule_decision(route,x,ts,event_start_ts):
    ...
    if event_key not in paper_consumed_events and x.get("max_age",999999)<=PRIMARY_BBO_AGE_MS and x.get("bbo_skew",999999)<=PRIMARY_MAX_SKEW_MS:
    ...
    while True:
        ...
        with pending_lock:
            ...
            for q in pending_paper_trials:
                ...
                (paper_due if due_ts<=ts else pkeep).append(q)
        ...
        for q in paper_due:
            if q.get("stage",1)==1:
                if paper_leg1(q,ts,books,meta):
                    with pending_lock: pending_paper_trials.append(q)
                else:
                    with paper_lock:
                        s=q["start_asset"]; paper_reserved[s]=max(0.0,paper_reserved.get(s,0.0)-q["reserved_units"])
                        paper["skipped_flash"]+=1; persist_paper()
            else:
                settle_paper_trade(q,ts,books,meta)
...
def raw_primary_status():
    ...
    r=con.execute("""SELECT COUNT(*) n,
      ...
      WHERE t.decision_ts_ms>=? AND t.decision_ts_ms<? AND t.latency_ms=? AND t.bbo_age_limit_ms=300
        AND d.recv_age_max_ms<=? AND d.recv_skew_ms<=?""",
...
<script>async function refresh(){let d=await fetch('/api/status').then(r=>r.json()),g=d.diag,s=d.sim,r=d.raw_primary||{};document.getElementById('cards').innerHTML=`<div class=c><div class=v>${d.symbols}</div><div class=muted>marchés WS</div></div><div class=c><div class=v>${d.routes}</div><div class=muted>routes 2-leg</div></div><div class=c><div class=v>${d.ws}/${d.ws_expected}</div><div class=muted>WebSockets</div></div><div class=c><div class=v>${d.age_ms??'-'} ms</div><div class=muted>dernier BBO reçu</div></div><div class=c><div class=v>${d.v22?.decisions||0}</div><div class=muted>décisions tracées</div></div><div class=c><div class=v>${d.v22?.trace_points||0}</div><div class=muted>points trajectoire</div></div><div class=c><div class=v>${d.v22?.avg_ws_transport_ms==null?'-':d.v22.avg_ws_transport_ms.toFixed(1)+' ms'}</div><div class=muted>MEXC → VPS brut (horloges non corrigées)</div></div>`;
let bals=Object.entries(s.balances||{}).map(([k,v])=>`${k}: ${v.toFixed(2)}`).join(' · ');document.getElementById('paper').innerHTML=`<div class=tag>SIMULATION CAPITAL RÉEL</div><h3>Paper bot — capital initial ${s.capital.toFixed(0)} $</h3><div class='v big ${s.profit>=0?'good':'bad'}'>${s.profit>=0?'+':''}${s.profit.toFixed(2)} $</div><div class='v ${s.return_pct>=0?'good':'bad'}'>${s.return_pct>=0?'+':''}${s.return_pct.toFixed(3)} %</div><div class=muted>NAV actuelle : ${s.nav.toFixed(2)} $ · profits/pertes réalloués automatiquement au capital</div><div class=cards style='margin-top:12px'><div><div class=v>${s.trades}</div><div class=muted>trades exécutés</div></div><div><div class=v>${s.rebalances}</div><div class=muted>rebalances</div></div><div><div class=v>${s.rebalance_cost.toFixed(3)} $</div><div class=muted>coût rebalance</div></div><div><div class=v>${s.skipped_balance}</div><div class=muted>refus capital</div></div></div><div class=muted style='margin-top:10px'>Buckets : ${bals}<br>Règles live : âge BBO local ≤${d.primary_age_ms} ms · skew réception ≤${d.primary_max_skew_ms} ms · BBO niveau 1 uniquement · leg 1 vers +${Math.floor(d.primary_latency_ms/2)} ms · leg 2 vers +${d.primary_latency_ms} ms · capital réservé à T0 · 1 entrée max par événement.</div>`;
document.getElementById('raw').innerHTML=`<div class=tag>DONNÉES BRUTES — SANS CAPITAL</div><h3>Même filtre bot, taille BBO brute</h3><div class='v big ${(r.pnl||0)>=0?'good':'bad'}'>${(r.pnl||0)>=0?'+':''}${(r.pnl||0).toFixed(2)} $</div><div class=muted>Pas de capital initial, pas de contrainte de buckets : ce PnL sert uniquement à comparer le signal brut à ce que les 2 000 $ peuvent réellement exploiter.</div><div class=cards style='margin-top:12px'><div><div class=v>${r.executed||0}</div><div class=muted>résolues</div></div><div><div class=v>${r.wins||0}</div><div class=muted>gagnantes</div></div><div><div class=v>${r.losses||0}</div><div class=muted>perdantes</div></div><div><div class=v>${r.avg_exec_net==null?'-':(100*r.avg_exec_net).toFixed(4)+'%'}</div><div class=muted>net exec moyen</div></div></div><div class=muted style='margin-top:10px'>Même filtre T0 : âge BBO local ≤${d.primary_age_ms} ms · skew réception ≤${d.primary_max_skew_ms} ms · PnL brut observé à +${d.primary_latency_ms} ms.</div>`;
let m=d.matrix||[],h='';for(let x of m){h+=`<tr><td>${x.age} ms</td><td>${x.lat} ms</td><td>${x.n}</td><td>${x.executed}</td><td>${x.wins||0}</td><td>${x.losses||0}</td><td class='${x.pnl>=0?'good':'bad'}'>${x.pnl.toFixed(3)} $</td><td>${x.avg_exec_net==null?'-':(100*x.avg_exec_net).toFixed(4)+'%'}</td></tr>`}document.getElementById('matrix').innerHTML=h;let rh='';for(let q of (d.recent_decisions||[])){let t=new Date(q.ts_ms);rh+=`<tr><td>${t.toLocaleTimeString()}</td><td>${q.route_id}</td><td class='good'>${(100*q.decision_net).toFixed(4)}%</td><td>${q.decision_size_usd.toFixed(2)} $</td><td>${q.recv_age_max_ms} ms</td><td>${q.recv_skew_ms} ms</td></tr>`}document.getElementById('recent').innerHTML=rh;document.getElementById('diag').innerHTML=`<h3>Univers</h3><div class=cards><div><div class=v>${g.all_markets}</div><div class=muted>marchés découverts</div></div><div><div class=v>${g.candidate_2leg}</div><div class=muted>2-leg candidates</div></div><div><div class=v>${g.selected_2leg}</div><div class=muted>2-leg suivies</div></div><div><div class=v>${g.candidate_3leg}</div><div class=muted>3-leg détectées mais ignorées</div></div></div>`}refresh();setInterval(refresh,3000)</script></body></html>'''
...
def bootstrap():
    ...
    put_db(("meta",("version",VERSION))); put_db(("meta",("market_depth_mode","top_of_book_only_no_rest_snapshot"))); put_db(("meta",("ws_bookticker_interval","10ms"))); put_db(("meta",("trace_window_ms",str(TRACE_WINDOW_MS)))); put_db(("meta",("market_source",state["market_source"])))
