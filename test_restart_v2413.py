...
spec=importlib.util.spec_from_file_location('restart',Path(__file__).resolve().parent/'relancer_mexc_v2413.py')
...
def data():
 ...
 if VERSION.startswith('2.4.13') and os.environ.get('FAKE_DIFFERENT')=='1':lv['cap_usd']=999
...
if __name__=='__main__':
 if VERSION.startswith('2.4.13') and os.environ.get('FAKE_EXIT')=='1':raise SystemExit(3)
...
class RestartTests(unittest.TestCase):
 ...
 def start(self,**flags):
  ...
  (self.root/'app.py').write_text(FAKE.replace('__VERSION__','2.4.12-bounded-live-session'))
 ...
 def test_busy_trade_prevents_termination(self):
  ...
  self.assertFalse((self.root/'scanner_v2413_live.pid').exists())
