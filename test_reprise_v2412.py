...
import reprendre_live_v2412 as r
...
def data():
 ...
 bad=VERSION.startswith('2.4.12') and os.environ.get('FAKE_BAD_FLOW')=='1'
 if VERSION.startswith('2.4.12') and os.environ.get('FAKE_BAD_LIMIT')=='1':lv['bbo_age_ms']=999
...
if __name__=='__main__':
 if VERSION.startswith('2.4.12') and os.environ.get('FAKE_EXIT')=='1':raise SystemExit(3)
 if VERSION.startswith('2.4.12') and os.environ.get('FAKE_TOUCH_STOP')=='1':os.utime(os.environ['LIVE_STOP_FILE'],None)
...
class ResumeTests(unittest.TestCase):
 ...
 def start(self,**flags):
  ...
  (self.root/'app.py').write_text(FAKE.replace('__VERSION__',r.OLD_VERSION))
 ...
 def test_preview_changes_no_gate_circuit_or_process(self):
  ...
  self.assertEqual(self.arm.stat().st_mtime_ns,mtime);self.assertTrue(self.read()['live']['circuit_open'])
 def test_explicit_session_arms_only_after_acknowledgement_and_health(self):
  ...
  newpid=int((self.root/'scanner_v2412_live.pid').read_text())
