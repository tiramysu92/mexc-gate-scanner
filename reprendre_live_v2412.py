#!/usr/bin/env python3
"""Reprise explicite et limitée du scanner V2.4.11 après CTO régularisé.
...
OLD_VERSION = '2.4.11-guarded-exit-recovery'
...
def monitor(read, stop, root, session_id, traces, db, created_ts_ms):
    ...
    try:
        ...
        try:
            with open(stop, 'x') as f: f.write('Fin de surveillance V2412 : nouvelles entrées suspendues.\n')
...
def resume(pid, root=ROOT, python=PYTHON, budget=None, minutes=30, apply=False,
           drain_timeout=30, startup_timeout=180, stable_seconds=30, watch=True):
    ...
    with open(root / '.v248_restart.lock', 'a') as lock:
        ...
        try:
            ...
            require(before.get('version') in (OLD_VERSION,VERSION), 'Le scanner actif doit être V2.4.11 ou V2.4.12.')
            ...
            with open(root/'scanner_v2412_live.log','ab',buffering=0) as log:
                child = subprocess.Popen(args,cwd=root,env=env,stdin=subprocess.DEVNULL,stdout=log,
                    stderr=subprocess.STDOUT,start_new_session=True,close_fds=True)
            (root/'scanner_v2412_live.pid').write_text(str(child.pid)+'\n')
