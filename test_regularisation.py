"""Manual accounting checks on an isolated SQLite journal, no exchange access."""
import hashlib
import json
...
class AccountingTests(unittest.TestCase):
    def setUp(self):
        ...
        fixture=json.loads((HERE/'fixtures/cto_journal_fixture.json').read_text())
    ...
    def test_preview_changes_nothing(self):
        ...
        self.assertEqual(plan['realized_pnl_usdt'],'-12.68668593664')
    ...
    def test_apply_preserves_orders_circuits_and_exact_external_proceeds(self):
        ...
        self.assertEqual(status,'manual_closed');self.assertAlmostEqual(net,7.28336738);self.assertAlmostEqual(loss,-12.68668593664)
    ...
    def test_manual_loss_is_counted_in_bot_daily_stats(self):
        ...
        m=self.m;m.initialize_live_journal();m.local_day_bounds_ms=lambda x:(0,1,'2026-09-14')
        try:
            m.refresh_live_daily_stats()
            self.assertAlmostEqual(m.live_runtime['realized_pnl'],-12.68668593664)
