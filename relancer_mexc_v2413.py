...
def restart(pid, root=ROOT, python=PYTHON, drain_timeout=30,
    ...
    with open(root / '.v248_restart.lock', 'a') as lock:
        ...
        try:
            ...
            before = read()
            require(before.get('version') in ('2.4.12-bounded-live-session', VERSION),
                    'La version active doit être V2.4.12 ou V2.4.13.')
            ...
            require(prepared.get('version') == VERSION and prepared.get('backend') in ('upb', 'cpp'),
                    'V2.4.13 et Protobuf natif doivent être présents avant l’arrêt.')

            stop = Path(env.get('LIVE_STOP_FILE', '/home/ubuntu/.config/mexc-bot/LIVE_STOP'))
            token = ('V2413 stopped restart ' + uuid.uuid4().hex + '\n').encode()
            owned = None
            try:
            ...
            try:
                ...
                print('Ancien processus terminé. Démarrage de V2.4.13.', flush=True)
                logfile = root / 'scanner_v2413_live.log'
                ...
                current = None
                while time.monotonic() < deadline:
                    require(child.poll() is None, 'Le nouveau processus s’est arrêté ; consulter scanner_v2413_live.log.')
                    try:
                        candidate = read()
...
def main():
    ...
    parser.add_argument('--pid', type=int, help='PID V2.4.12 ; sinon détection du processus unique dans ce projet')
