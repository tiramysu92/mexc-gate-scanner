...
def restart(pid, root=ROOT, python=PYTHON, drain_timeout=30,
    ...
    with open(root / '.v248_restart.lock', 'a') as lock:
        ...
        try:
            ...
            before = read()
            require(before.get('version') == '2.4.7-sized-fresh-execution',
                    'La version active n’est pas V2.4.7 ; aucune relance automatique.')
            ...
            require(prepared.get('version') == VERSION and prepared.get('backend') in ('upb', 'cpp'),
                    'V2.4.8 et Protobuf natif doivent être présents avant l’arrêt.')
            ...
            token = ('V248 restart ' + uuid.uuid4().hex + '\n').encode()
            owned = None
            try:
            ...
            try:
                ...
                print('Ancien processus terminé. Démarrage de V2.4.8.', flush=True)
                logfile = root / 'scanner_v248_live.log'
                ...
                (root / 'scanner_v248_live.pid').write_text(str(child.pid) + '\n')
                print(f'Nouveau PID : {child.pid} ; journal : {logfile.name}', flush=True)
                deadline = time.monotonic() + startup_timeout
                current = None
                while time.monotonic() < deadline:
                    require(child.poll() is None, 'Le nouveau processus s’est arrêté ; consulter scanner_v248_live.log.')
                    try:
                        candidate = read()
...
def main():
    ...
    parser.add_argument('--pid', type=int, required=True, help='PID V2.4.7 relevé sur ce serveur')
